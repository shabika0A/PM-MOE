...
class ToPMoE(nn.Module):
    ...
    def __init__(self, trained_experts: list[nn.Module], gate_input_dim: int, args):
        ...
        # Initialize the UCB experts selector
        self.ucb_selector = UCBExperts(num_experts=self.num_experts, exploration_constant=args.ucb_c) # You'll need to add a ucb_c argument to your args



        # You might still want to keep the gating network for weighting
...
#         self.trained_experts = trained_experts
    ...
    #         return weight_out_t 
        ...
        return torch.sum(final_results * weights, dim=1)
